...
async function retryWithBackoff<T>(
  ...
  try {
    ...
    // Check if error is retryable (overloaded, rate limit, network errors)
    const errorMessage = error instanceof Error ? error.message : String(error);
    const isRetryable =
      errorMessage.includes('overloaded') ||
      errorMessage.includes('Overloaded') ||
      (error && typeof error === 'object' && 'status' in error && (error.status === 429 || error.status === 503)) ||
      (error && typeof error === 'object' && 'code' in error && (error.code === 'ECONNRESET' || error.code === 'ETIMEDOUT'));
...
Generate a comprehensive summary following the structure provided.`;

    // Determine provider and model
    const provider = process.env.AI_PROVIDER === 'minimax' ? 'minimax' : 'anthropic';
    const model = provider === 'minimax'
      ? minimax('abab6.5s-chat')
      : anthropic('claude-sonnet-4-5-20250929');

    // Stream the response from AI with retry logic
          ...
          try {
            ...
            const { data: summaryData, error: summaryError } = await supabaseAdmin
              ...
              .insert({
                ...
                summary_text: text,
                metadata: {
                  model: provider === 'minimax' ? 'minimax-abab6.5s-chat' : 'claude-sonnet-4-5-20250929',
            ...
            // Extract and save topics from the "Topics & Tags" section
            const topicsMatch = text.match(/##?\s*5\.\s*\*\*Topics\s*&\s*Tags\*\*[\s\S]*?(?=\n##|$)/i);
            if (topicsMatch && summaryData) {
              const topicsSection = topicsMatch[0];
              const topicLines = topicsSection
                .split('\n')
              ...
              if (topicLines.length > 0) {
                const topicsToInsert = topicLines.map(topic => ({
