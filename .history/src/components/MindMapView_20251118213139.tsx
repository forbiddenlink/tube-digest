...
const nodeTypes = {
  root: ({ data }: { data: { label: string } }) => (
    <div className="px-6 py-4 rounded-lg bg-gradient-to-r from-purple-500 to-pink-500 text-white font-bold text-lg shadow-lg">
      {data.label}
    </div>
  ),
  ...
  topic: ({ data }: { data: { label: string } }) => (
    <div className="px-3 py-2 rounded-full bg-gradient-to-r from-orange-400 to-red-400 text-white text-sm shadow-md">
      {data.label}
    </div>
  ),
};

export default function MindMapView({ summaryId }: MindMapViewProps) {
