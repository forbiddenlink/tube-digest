...
// System prompt with caching for 90% cost savings
const SYSTEM_PROMPT = {
  type: 'text' as const,
...
export async function POST(req: Request) {
  try {
    ...
    // Get video title (basic extraction from first few words)
    const videoTitle = transcript.slice(0, 100).split(' ').slice(0, 10).join(' ') + '...';
