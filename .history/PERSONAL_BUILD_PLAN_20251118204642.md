...
### System Prompt (Cached for 90% Savings)

```typescript
const SYSTEM_PROMPT = {
  type: 'text',
  text: `You are a technical learning assistant for developers. Generate structured summaries that help quickly understand and recall video content.
...
1. **Consistent Structure** - Always get the same JSON format for reliable parsing
2. **Prompt Caching** - System prompt cached = $0.003 vs $0.03 per summary (90% savings)
3. **Rich Context** - Video metadata helps Claude understand context better
4. **Actionable Output** - Not just "what" but "why" and "when to use"
5. **Searchable** - Structured fields make semantic search more effective
...
| Challenge | Solution | Priority | Timeline |
|-----------|----------|----------|----------|
| **YouTube transcripts unavailable** | Fallback to Whisper API ($0.006/min) | High | Week 2 |
| **Rate limiting (Anthropic)** | Queue system, exponential backoff, show position | Medium | Week 3 |
| **Large transcripts (>100k tokens)** | Chunk + map-reduce summary strategy | Medium | Week 4 |
| **Mind map layout performance** | Web Worker for layout, lazy rendering, viewport culling | Low | Week 5 |
| **Mobile mind map UX** | Simplified view, pinch-zoom, auto-layout | Low | Week 6 |
| **Search quality on small dataset** | Hybrid search + BM25 fallback, typo tolerance | Medium | Week 3 |
| **API costs spike** | Usage alerts, monthly budgets, cache aggressively | High | Week 1 |
| **Duplicate videos** | Check by video ID before processing, show existing | Medium | Week 2 |

### Prototype Strategy

- **Manual fallbacks first** - Copy-paste transcript if API fails
- **Automate after validating** - Prove the workflow works manually first
- **Optimistic UI** - Show immediate feedback, add error handling iteratively
- **Test with diversity** - Try 10min, 1hr, 3hr videos with different content types

### Testing Approach

- **Unit tests** - Prompt formatting, search algorithms, data transforms
- **Integration tests** - API routes, database queries, streaming responses
- **E2E tests (Playwright)** - Critical user flows only (add video, search, view)
- **Manual testing** - Mind map layouts, edge cases, mobile UX, accessibility
...
## 💻 Your First API Route (Day 1)

Get to a working MVP in <2 hours:
...
export async function POST(req: NextRequest) {
  ...
  const message = await anthropic.messages.create({
    ...
    messages: [{
      role: 'user',
      content: `Summarize this YouTube video concisely:\n\n${text.slice(0, 100000)}`
    }]
  });
...
**That's it!** 30 lines to a working MVP. From here:

- ✅ Week 1: Add streaming, save to database
- ✅ Week 2: Build UI, add search
- ✅ Week 3-4: Mind maps, GitHub sync
